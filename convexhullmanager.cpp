...
bool ConvexHullManager::_is_colinear(const QPointF& p1, const QPointF& p2, const QPointF& p) const
{
    ...
    if((std::fabs(p.x() - p1.x()) < 0.001 && std::fabs(p.x() - p2.x()) < 0.001)
       ||(std::fabs(p.y() - p1.y()) < 0.001 && std::fabs(p.y() - p2.y()) <0.001))
    {
        return true;
    }
    if(std::fabs(((p.x() - p1.x()) / (p.y() - p1.y())) - ((p2.x() - p.x())/(p2.y() - p.y()))) < 0.0001 )
    {
        return true;
    }
    return false;
